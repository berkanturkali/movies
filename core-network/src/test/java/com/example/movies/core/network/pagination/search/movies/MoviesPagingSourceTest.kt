...
class MoviesPagingSourceTest {
    ...
    fun `check that prev and next keys are correct`() = runBlocking {
        val pagingSource = MoviesPagingSource(remoteDataSource, null)
        val movies = remoteDataSource.fetchMovies(1, null)
        Truth.assertThat(
    ...
    fun `check that paging source returns error when error occurs`() = runBlocking {
        ...
        coEvery { remoteDataSource.fetchMovies(1, null) } throws Exception()
        val pagingSource = MoviesPagingSource(remoteDataSource, null)
