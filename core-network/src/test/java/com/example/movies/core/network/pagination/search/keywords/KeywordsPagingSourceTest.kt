...
class KeywordsPagingSourceTest {
    ...
    fun `check that prev and next keys are correct`() = runBlocking {
        val pagingSource = KeywordsPagingSource(remoteDataSource, null)
        val keywords = remoteDataSource.fetchKeywords(1, null)
        Truth.assertThat(
    ...
    fun `check that paging source returns error when error occurs`() = runBlocking {
        ...
        coEvery { remoteDataSource.fetchKeywords(1, null) } throws Exception()
