...
class CompaniesPagingSourceTest {
    ...
    fun `check that prev and next keys are correct`() = runBlocking {
        val pagingSource = CompaniesPagingSource(remoteDataSource, null)
        val companies = remoteDataSource.fetchCompanies(1, null)
        Truth.assertThat(
    ...
    fun `check that paging source returns error when error occurs`() = runBlocking {
        ...
        coEvery { remoteDataSource.fetchCompanies(1, null) } throws Exception()
        val pagingSource = CollectionsPagingSource(remoteDataSource, null)
