...
class PeoplePagingSourceTest {
    ...
    fun `check that prev and next keys are correct`() = runBlocking {
        val pagingSource = PeoplePagingSource(remoteDataSource, null)
        val people = remoteDataSource.fetchPeople(1, null)
        Truth.assertThat(
    ...
    fun `check that paging source returns error when error occurs`() = runBlocking {
        ...
        coEvery { remoteDataSource.fetchPeople(1, null) } throws Exception()
        val pagingSource = PeoplePagingSource(remoteDataSource, null)
